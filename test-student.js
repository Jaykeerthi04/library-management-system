...
async function test() {
  try {
    ...
    console.log("\n✓ Test 1: Login as Student (After Fix)");
    const loginRes = await axios.post(`${API_URL}/auth/login`, {
      email: "priya@student.com",
      password: "pass123",
    });
    studentId = loginRes.data._id;
    studentToken = loginRes.data.token;
    console.log(`  Student: ${loginRes.data.name}`);
    ...
    // Test 3: Student Borrows a Book (SHOULD NOW WORK with fix)
    console.log("\n✓ Test 3: Student Borrows a Book");
    ...
    console.log(`  ✅ Success! Student can now borrow books`);
    ...
    // Test 5: Check Fines (should be 0 as no overdue)
    console.log("\n✓ Test 5: Check Fines");
    ...
    console.log(`  ✅ Success! Student can now return books`);
    ...
    console.log("\n✅ ALL TESTS PASSED! Authorization bug fixed successfully!");
  } catch (error) {
    console.error("\n❌ Test failed:");
    if (error.response) {
